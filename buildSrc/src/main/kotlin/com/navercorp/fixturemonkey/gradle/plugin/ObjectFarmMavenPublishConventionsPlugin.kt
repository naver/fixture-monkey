...
class ObjectFarmMavenPublishConventionsPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        project.extensions.getByType<PublishingExtension>().apply {
            publications {
                register<MavenPublication>("objectFarmApi") {
                    ...
                    val objectFarmVersion = project.findProperty("objectFarmVersion") as String
                    version = objectFarmVersion

                    pom {
                        name.set("object-farm-api")
                        description.set("API for navigating and manipulating object graphs")
        ...
        project.extensions.getByType<SigningExtension>().apply {
            ...
            // Use objectFarmVersion for signing decision, fallback to project version
            val objectFarmVersion: String? by project
            val versionToCheck = objectFarmVersion ?: project.version as? String
            
            if (versionToCheck?.endsWith("SNAPSHOT") == false &&
                !runningTaskNames.contains("publishToMavenLocal")
            ) {
                sign(project.extensions.getByType<PublishingExtension>().publications["objectFarmApi"])
            }
        }
    }
}

...
`defaultNotNull`과 `defaultNullInjectGenerator` 옵션으로 null 생성을 제어할 수 있습니다:
...
FixtureMonkey fixtureMonkey = FixtureMonkey.builder()
    .defaultNotNull(true) // null 가능으로 선언되지 않은 프로퍼티는 non-null
    .build();

// @Nullable 프로퍼티까지 포함해 null을 아예 만들지 않는 FixtureMonkey 생성
FixtureMonkey fixtureMonkey = FixtureMonkey.builder()
...
// String? 같은 nullable 타입까지 포함해 null을 아예 만들지 않는 FixtureMonkey 생성
val fixtureMonkey = FixtureMonkey.builder()
...
`defaultNotNull(true)`은 스스로 null 가능하다고 선언한 프로퍼티는 건드리지 않습니다 - Java의 `@Nullable` 어노테이션이나 Kotlin의 `String?` 같은 타입이 여기 해당하며, 여전히 약 20% 확률로 null이 됩니다. [기본 NotNull 설정](../fixture-monkey-options/essential-options-for-beginners#기본-notnull-설정)을 참조하세요.

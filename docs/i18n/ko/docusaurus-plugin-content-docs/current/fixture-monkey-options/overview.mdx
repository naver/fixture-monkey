...
FixtureMonkey fixtureMonkey = FixtureMonkey.builder()
    .defaultNotNull(true)  // null 가능으로 선언된 프로퍼티를 제외하면 null 없음
...
:::note
아래 Java 예시는 `objectIntrospector(ConstructorPropertiesArbitraryIntrospector.INSTANCE)`도 함께 설정합니다. 예시에서 사용하는 `Product` 클래스가 Lombok `@Value`를 사용하기 때문입니다(`@Value`는 `@ConstructorProperties`가 붙은 전체 인자 생성자를 생성합니다). 실제 클래스 유형에 맞는 인트로스펙터를 선택하세요.
:::

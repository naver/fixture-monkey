...
#### Bug Fixes
- Kotlin에서 `Type annotation class 'org.checkerframework.checker.nullness.qual.UnknownKeyFor' of the inferred type is inaccessible` 오류로 컴파일이 실패하던 문제 수정. 배포되는 클래스 파일에 Checker Framework가 추론한 어노테이션이 더 이상 포함되지 않습니다
...
#### Bug Fixes
- 객체를 통째로 set할 때 Kotlin value class 프로퍼티 값이 유실되는 문제 수정
- `instantiateBy`로 생성할 때 nullable Kotlin value class 프로퍼티가 null인 객체를 set하면 NPE가 발생하는 문제 수정
...
#### Improvements
- Kotlin primary constructor fixture 생성 최적화
- Jackson 의존성을 2.18.8로 업그레이드
...
- `FixtureMonkeyOptions.toBuilder()`가 모든 필드를 복사하도록 수정
...
#### Bug Fixes
- `instantiateBy` 사용 시 어노테이션 조회에서 NPE 발생 수정
- size validation 오류 메시지 오타 수정
...
#### Bug Fixes
- 중첩 컨테이너의 제네릭 타입 해석 및 earlyReturn 컨테이너 경로에서의 와일드카드 오버라이드 수정

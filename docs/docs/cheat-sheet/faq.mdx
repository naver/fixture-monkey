...
You can control null generation with the `defaultNotNull` and `defaultNullInjectGenerator` options:
...
FixtureMonkey fixtureMonkey = FixtureMonkey.builder()
    .defaultNotNull(true) // Non-null unless a property is declared nullable
    .build();

// Create a FixtureMonkey with no null values at all, @Nullable properties included
FixtureMonkey fixtureMonkey = FixtureMonkey.builder()
...
FixtureMonkey fixtureMonkey = FixtureMonkey.builder()
    .defaultNullInjectGenerator(new DefaultNullInjectGenerator(
        0.5,                                              // null probability
        false,                                            // nullableContainer
        false,                                            // defaultNotNull
...
// Create a FixtureMonkey with no null values at all, nullable types such as String? included
val fixtureMonkey = FixtureMonkey.builder()
    .defaultNullInjectGenerator { 0.0 }
    .build()
...
val fixtureMonkey = FixtureMonkey.builder()
    .defaultNullInjectGenerator(
        DefaultNullInjectGenerator(
            ...
            false,                                      // defaultNotNull
            false,                                      // nullableElement
            HashSet(DEFAULT_NULLABLE_ANNOTATION_TYPES),
            HashSet(DEFAULT_NOTNULL_ANNOTATION_TYPES)
        )
    )
    .build()
...
`defaultNotNull(true)` leaves a property that declares itself nullable alone - a Java `@Nullable` annotation or a Kotlin type such as `String?` - so it is still null about 20% of the time. See [Default NotNull Setting](../fixture-monkey-options/essential-options-for-beginners#default-notnull-setting).

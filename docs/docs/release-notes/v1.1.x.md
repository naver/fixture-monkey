...
#### Bug Fixes
- Fix a Kotlin consumer failing to compile with `Type annotation class 'org.checkerframework.checker.nullness.qual.UnknownKeyFor' of the inferred type is inaccessible`. The published class files no longer carry the annotations the Checker Framework infers.
...
#### Bug Fixes
- Fix Kotlin value class property values being discarded when setting a whole object
- Fix NPE when setting an object whose nullable Kotlin value class property is null through `instantiateBy`
...
- Bump Jackson dependency to 2.18.8
...
- Fix `FixtureMonkeyOptions.toBuilder()` to copy all fields

...
Human readers looking for an introduction should start with [Creating objects](../get-started/creating-objects) instead.
...
| Page | Contents |
| :--- | :--- |
| [Writing tests](./writing-tests) | The decision procedure: how to tell which properties a scenario depends on, which API to pin them with, and how to verify the result |
| [API reference](./api-reference) | Condensed Java and Kotlin API surface — selectors, builder methods, setup |
...
These pages describe the current release line. The selector APIs (`javaGetter`, the Kotlin `Exp` extensions) are marked experimental and have moved packages before — check the [migration guide](../migration-guide/) if the project you are working in is pinned to an older version.

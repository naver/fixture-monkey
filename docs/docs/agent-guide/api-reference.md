...
Condensed API surface for agents. For the reasoning about *which* of these to use, see [Writing tests](./writing-tests).
...
| `setInner(innerSpec)` | Customize maps and other structures path expressions cannot reach. See [InnerSpec](../customizing-objects/innerspec) |
| `instantiate(instantiator)` | Choose a constructor or factory method. See [Instantiate methods](../generating-objects/instantiate-methods) |
...
| Interfaces, abstract classes, sealed types | — | `new InterfacePlugin()`; see [Interface plugin](../plugins/interface-plugin/features) |
...
See [Introspector](../generating-objects/introspector) and [Instantiate methods](../generating-objects/instantiate-methods) for the full treatment.
...
See [Fixture Monkey options](../fixture-monkey-options/overview) for the full option set, and the Plugins section for each plugin — [Kotlin](../plugins/kotlin-plugin/features) is the one most agents need.

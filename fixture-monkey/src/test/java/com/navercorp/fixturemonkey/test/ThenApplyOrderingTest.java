...
class ThenApplyOrderingTest {
	...
	void setDecomposedThenApplyIndexSizeAndWildcardSize() {
		...
		then(actual.get(0).getValues().get(0)).isEqualTo("1");
		then(actual.get(0).getValues().get(1)).isEqualTo("2");
		then(actual.get(0).getValues().get(2)).isEqualTo("3");
	...
	@Test
	void wildcardSizeAfterExactOverridesExact() {
		// when
		List<List<String>> actual = SUT.giveMeBuilder(ListListStringObject.class)
			.size("values", 2)
			.size("values[0]", 1)
		...
		// then
		then(actual).hasSize(2);
		then(actual.get(0)).hasSize(5);
		then(actual.get(1)).hasSize(5);
	}

	@Test
	void exactSizeAfterWildcardWinsAtIndex() {
		// when
		List<List<String>> actual = SUT.giveMeBuilder(ListListStringObject.class)
			.size("values", 2)
			.size("values[*]", 5)

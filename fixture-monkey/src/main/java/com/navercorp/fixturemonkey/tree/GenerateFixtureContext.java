...
public final class GenerateFixtureContext implements TraverseNodeContext {
	...
	private LazyArbitrary<Boolean> childManipulated = null;
	...
	private static boolean initializeChildNotCacheable(ObjectNode objectNode) {
		for (ObjectNode child : objectNode.resolveChildren().asList()) {
			GenerateFixtureContext childGenerateFixtureContext = child.getObjectNodeContext();
			if (childGenerateFixtureContext.childManipulated.getValue()
	...
	private static boolean initializeChildManipulated(ObjectNode objectNode) {
		for (ObjectNode child : objectNode.resolveChildren().asList()) {
			if (child.manipulated() || child.getObjectNodeContext().childManipulated.getValue()) {
				return true;
			}
		}
	...
	public ArbitraryGeneratorContext generateContext(
		...
		List<ObjectNode> children = objectNode.resolveChildren().asList().stream()

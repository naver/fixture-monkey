...
final class RootArbitrary<T> implements CombinableArbitrary<T> {
		...
		for (int i = 0; i < generateMaxTries; i++) {
			try {
				return arbitrary.getValue()
					...
					.combined();
			} catch (ContainerSizeFilterMissException | RetryableFilterMissException
				| FixedValueFilterMissException ex) {
				lastException = ex;
			} finally {
				arbitrary.clear();
			}

...
class PropertySelectorAdapterTest {
	...
	void nestedTypedJavaGetter() {
		...
		String actual = SUT.giveMeBuilder(RootJavaTypeObject.class)
			.customizeProperty(javaGetter(RootJavaTypeObject::getValue).into(JavaTypeObject::getString), arb ->
				arb.map(it -> expected)
			)
			.sample()
			.getValue()
	...
	void indexTypedJavaGetter() {
		...
		String actual = SUT.giveMeBuilder(ContainerObject.class)
			.size("list", 1)
			.customizeProperty(javaGetter(ContainerObject::getList).index(String.class, 0), arb ->
				arb.map(it -> expected)
			)
			.sample()
			.getList()
